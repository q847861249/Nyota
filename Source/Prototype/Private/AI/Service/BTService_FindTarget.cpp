...
void UBTService_FindTarget::TickNode(UBehaviorTreeComponent &OwnerComp, uint8 *NodeMemory, float DeltaSeconds)
{
    ...
    ABaseEnemy* EC = Cast<ABaseEnemy>(EnemyController->GetPawn());
    if(!EC) return;
    ...
    TArray<ABasePlayer*> BasePlayerArr;
    //filter overlap result, if result has player character add it in array
    for(const FOverlapResult& Result : OverlapResults)
    {
        if(!IsValid(Result.GetActor())) continue;
        ABasePlayer* PlayerCharacterActor = Cast<ABasePlayer>(Result.GetActor());
        if(PlayerCharacterActor) BasePlayerArr.Add(PlayerCharacterActor);
    
    }
    //Currently only use the first result as target
    if (BasePlayerArr.Num() > 0) BC->SetValueAsObject(GetSelectedBlackboardKey(),BasePlayerArr[0]);
    ...
    {
        ABasePlayer* PC = Cast<ABasePlayer>(BC->GetValueAsObject(GetSelectedBlackboardKey()));
    ...
    DrawDebugSphere(GetWorld(),EnemyController->GetPawn()->GetActorLocation(),BC->GetValueAsFloat(FName("AttackRange")),12,FColor::Yellow,false,0.5f);
}
